...
function replayAnalyticsRequests() {
  ...
  getObjectStore(STORE_NAME, 'readonly').openCursor().onsuccess = function(event) {
    ...
    if (cursor) {
      ...
      console.log('About to replay %d saved smartMailOne Analytics requests...', savedRequests.length);
...
function checkForAnalyticsRequest(requestUrl) {
  ...
  if ((url.hostname == 'https://github.com/srenzo/smartmailOne.git') &&
       url.pathname == '/collect') {
    console.log('  Storing smartMailOne Analytics request in IndexedDB to be replayed later.');
